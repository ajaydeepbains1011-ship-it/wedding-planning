...
const DEFAULT_KANBAN = {
  todo: [
    ...
    { id:"k6",  title:"Find & book Bhai Ji",                    event:"wedding",   priority:"high", due:"2026-10" },
...
const DEFAULT_CHECKLIST = [
  ...
  { category:"Vendors (Core)", items:[
    ...
    {id:"c24", text:"Bhai Ji confirmed for wedding ceremony", done:false},
...
export default function WeddingPlanner() {
        ...
        <div style={{display:"flex",gap:0,overflowX:"auto",maxWidth:1240,margin:"6px auto 0",msOverflowStyle:"none",scrollbarWidth:"none"}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} className={`nav-tab${tab===t.id?" active":""}`}
              style={{fontSize:isMobile?14:13,padding:isMobile?"14px 16px":"11px 16px", minHeight:isMobile?48:undefined}}>
              {t.label}
            </button>
          ))}
        </div>
  ...
  gap:isMobile?10:14
      ...
      <div style={{padding:isMobile?"14px 12px 80px":"28px 28px 80px",maxWidth:1240,margin:"0 auto"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
              {[
                ...
                <div key={s.label} className="card" style={{padding:"14px 18px"}}>
                  ...
                  <div style={{fontSize:20,fontWeight:700,color:s.col||"#0f0f1a",marginTop:3}}>{s.val}</div>
        ...
        {tab==="budget" && (
          <div>
            <div style={{marginBottom:18,padding:isMobile?"18px 18px":"22px 24px",borderRadius:20,background:"linear-gradient(135deg,rgba(79,70,229,0.10),rgba(124,58,237,0.08),rgba(14,165,233,0.10))",border:"1px solid rgba(79,70,229,0.16)",boxShadow:"0 10px 30px rgba(79,70,229,0.08)"}}>
              <div style={{fontSize:11,fontWeight:800,color:"#4F46E5",textTransform:"uppercase",letterSpacing:1.2,marginBottom:6}}>Wedding Money Tracker</div>
              <div style={{fontSize:isMobile?24:30,fontWeight:900,color:"#0f0f1a",letterSpacing:-1.1,lineHeight:1.05}}>Know exactly where the money is going.</div>
              <div style={{fontSize:isMobile?13:14,color:"#4a4a6a",marginTop:8,lineHeight:1.55,maxWidth:720}}>Pick an event below, adjust each line item, and keep payments separate in the Payments tab. The color cards show what is allocated, paid, and still remaining.</div>
            </div>
            <div style={{...card(),marginBottom:18,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",borderRadius:18,border:"1px solid rgba(79,70,229,0.14)",boxShadow:"0 10px 26px rgba(79,70,229,0.08)"}}>
              <div><div style={{fontSize:10,fontWeight:800,color:"#4F46E5",textTransform:"uppercase",letterSpacing:0.9}}>Allocated So Far</div><div style={{fontSize:isMobile?24:28,fontWeight:900,color:totalBudgetAll>500000?"#EF4444":"#0f0f1a",letterSpacing:-0.7}}>${totalBudgetAll.toLocaleString()}</div></div>
              <div style={{color:"#d1d5db",fontSize:18}}>/</div>
              <div><div style={{fontSize:10,letterSpacing:1,color:"#7C3AED",fontWeight:800,textTransform:"uppercase"}}>Target Budget</div><div style={{fontSize:isMobile?24:28,fontWeight:900,color:"#7C3AED",letterSpacing:-0.7}}>$500,000</div></div>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:18}}>
              {events.map(ev=>(
                <button key={ev.id} onClick={()=>setBudgetEvent(ev.id)} style={{background:budgetEvent===ev.id?ev.color:"#ffffff",color:budgetEvent===ev.id?"#fff":ev.color,border:`1.5px solid ${budgetEvent===ev.id?ev.color:ev.color+"55"}`,padding:isMobile?"12px 15px":"9px 16px",borderRadius:99,cursor:"pointer",fontSize:isMobile?13:12,fontFamily:"inherit",fontWeight:800,boxShadow:budgetEvent===ev.id?`0 8px 18px ${ev.color}33`:"0 2px 8px rgba(0,0,0,0.04)",minHeight:isMobile?46:undefined}}>
                  {ev.emoji} {ev.label}
                </button>
              ))}
            </div>
                ...
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:16}}>
                    {[
                      ...
                      {label:"Paid (USD)",val:fmtUSD(totalSpentUsd),col:"#B45309"},
                  ...
                  <button onClick={()=>addBudgetItem(budgetEvent)} style={{marginTop:12,background:"linear-gradient(135deg,rgba(79,70,229,0.08),rgba(124,58,237,0.06))",border:"1.5px dashed rgba(79,70,229,0.35)",color:"#4F46E5",borderRadius:14,padding:isMobile?"14px 16px":"12px 16px",cursor:"pointer",fontSize:isMobile?14:13,fontWeight:800,fontFamily:"inherit",width:"100%"}}>+ Add line item</button>
                </div>
              );
            })()}
          </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:16}}>
              {[
                ...
                <div key={s.label} className="card" style={{padding:"12px 16px"}}>
                  ...
                  <div style={{fontSize:22,fontWeight:700,color:s.col||"#0f0f1a",marginTop:3}}>{s.val}</div>
                </div>
              ))}
            </div>
